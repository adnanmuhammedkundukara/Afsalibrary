...
from Afsalibrary.database.autofilter_db import save_file
...
@Afsalibrary.on_callback_query(Worker.regex(r'^index'))
...
@Afsalibrary.on_message((Worker.forwarded | (Worker.regex("(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)$")) & Worker.text ) & Worker.private & Worker.incoming)
...
@Afsalibrary.on_message(Worker.command('setskip') & Worker.user(ADMINS))
