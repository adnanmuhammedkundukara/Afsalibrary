...
from Afsalibrary.database._utils import get_size, split_list
from Afsalibrary.database.autofilter_db import get_filter_results, get_poster
from config import BUTTONS, bot_info, SPELL_MODE, SET_SPEL_M, SUPPORT, BOT_PICS
from translation import Afsalibrary


#@Afsalibrary.on_message(Worker.text & Worker.group & Worker.incoming & Worker.chat(AUTH_GROUPS) if AUTH_GROUPS else Worker.text & Worker.group & Worker.incoming)
    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}", callback_data=f"Afsalibrary#{file_id}")]
                )
        else:
            if SPELL_MODE:
